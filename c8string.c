...
{
  struct c8string* so = to_c8string(o);
  assert(so);
  c8buf_clear(&so->buf);
  free(so);
}
...
{
  const struct c8string* so = to_const_c8string(o);
  assert(so);
  return (struct c8obj*)c8string_create_buf(&so->buf);
}
...
static struct c8obj* c8string_op(struct c8obj* o, int op, struct c8obj* p)
{
  ...
  switch (op) {
    ...
    case C8_OP_EQUALITY: case C8_OP_INEQUALITY: {
      ...
      c8obj_str(p, &bp, 0);
      int cmp = strcmp(c8buf_str(&so->buf), c8buf_str(&bp));
...
{
  struct c8string* so = malloc(sizeof(struct c8string));
  assert(so);
  so->base.refs = 1;
  so->base.imp = &c8string_imp;
  c8buf_init(&so->buf);
  return so;
}
...
{
  struct c8string* so = c8string_create();
  c8buf_init_str(&so->buf, str);
  return so;
}
