...
static struct c8obj* expression(struct c8expr* o, int p, int f, int ex);
static struct c8obj* primary(struct c8expr* o, int f, int ex);
static struct c8obj* list(struct c8expr* o, int lop, int ex);
static struct c8obj* resolve(struct c8expr* o, const char* name);

static void next(struct c8expr* o);
...
{
  struct c8expr* o = malloc(sizeof(struct c8expr));
  assert(o);
  o->global = global;
  o->local = 0;
  return o;
